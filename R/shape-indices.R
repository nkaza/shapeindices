...
.ALL_SHAPE_INDICES <- c(
    "convexity", "moment_of_inertia", "moment_isotropy", "directional_balance",
...
.resolve_which <- function(which) {
    if (identical(which, "all")) {
        return(.ALL_SHAPE_INDICES)
    }
    if (!is.character(which)) {
        stop(
            "`which` must be \"all\" or a character vector of index names; got ",
            class(which)[1], "."
        )
    }
    ...
    if (length(unknown) > 0) {
        stop(
            "Unknown index name(s) in `which`: ", paste(unknown, collapse = ", "),
...
#' @param already_projected passed to `prepare_polygon()` - see its own
#'   doc for the (real, but caller-verified-only) risk of setting this.
...
shape_indices <- function(poly, which = "all", deterministic_max_tri = NULL,
                          simplify_tolerance = NULL, already_projected = FALSE, ...) {
    which <- .resolve_which(which)
    needs_mesh <- any(.MESH_INDICES %in% which)
    prep <- if (needs_mesh) {
        prepare_polygon(poly,
...
#' rather than shrinking it (a real 50,424-row dataset that unions to ONE
#' clean ~50K-vertex polygon produces 1,851 disjoint sliver parts
#' instead, after independently simplifying every row first). Doing it
#' here, once, after the union already dissolved every shared internal
...
.resolve_union <- function(x, weights, simplify_tolerance = NULL) {
    ...
    if (any(is_hole)) {
        warning(
            sprintf("%d of %d rows have zero or NA weight; ", sum(is_hole), n_row),
    ...
    list(
        geoms = geoms[keep], w_row = w_row[keep], poly_u = poly_u,
...
.coerce_weight_vector <- function(v, what, code = what) {
    if (is.factor(v)) {
        stop(
            what, " is a factor; as.numeric() on a factor silently returns its integer ",
    ...
    w_row <- if (is.null(weights)) {
        ...
        if (!weights %in% names(x_attr)) {
            ...
            stop("weights = \"", weights, "\" is not a column of x", if (weights %in% names(x)) {
                paste0(
                    " - it's x's geometry column (", weights, "), not an attribute column, so ",
...
shape_indices_sf <- function(x, byrow = TRUE, which = "all", weights = NULL, id = NULL,
    ...
    if (byrow) {
        if (!is.null(weights)) {
            stop(
                "weights is only used when byrow = FALSE; with byrow = TRUE each row ",
                "is indexed on its own, unweighted."
            )
        }
        if (!is.null(id)) {
            stop(
                "id is only used when byrow = FALSE; with byrow = TRUE each row of x ",
    ...
    if (!missing(parallel_rows) && parallel_rows) {
        stop(
            "parallel_rows = TRUE has no meaning when byrow = FALSE - there are no ",
...
.shape_indices_sf_byrow <- function(x, which, parallel_rows, ...) {
    # project the WHOLE input once, same as constrained_mesh()/the byrow =
    # FALSE path already do - otherwise each row's own shape_indices() call
    # independently re-triggers .ensure_projected() on just that one
    # polygon, rebuilding a fresh local azimuthal CRS and reprojecting once
    # per row. That's a real cost on geographic input (~85% of total time,
    # almost entirely CRS setup + transform, not triangulation) and it's
    # pure waste - a single shared projection centred on the whole
    # dataset's own union is negligibly different for any one row (max
    # per-row index difference ~0.003 on real county-level data).
    # already_projected = TRUE below then skips each row's own
    # .ensure_projected() check entirely rather than just letting it fall
    # through as a no-op - st_is_longlat() itself re-parses the CRS from
    # scratch on every call (uncached), a real, separately measured cost
    # even when it correctly determines "already planar, nothing to do".
    x <- .ensure_projected(x)
    geoms <- st_geometry(x)
    dots <- c(list(...), list(which = which, already_projected = TRUE))
    ...
    if (use_parallel) {
        if (inherits(future::plan(), "sequential")) {
            warning(
                "parallel_rows = TRUE but no parallel future::plan() is active ",
...
.shape_indices_sf_grouped <- function(x, which, weights, id, ...) {
    if (is.null(id)) {
        stop(
            "byrow = FALSE collapses every row of x into a single result, so there is ",
    ...
    if (needs_mesh) {
        ...
        if (is.null(array_mesh)) {
            ...
            safe_ceiling <- min(
                .safe_deterministic_tri_ceiling(n_quad, "convexity"),
            ...
            if (n_tri <= safe_ceiling) {
                ...
                stop(sprintf(
                    paste(
                        "deterministic_max_tri = %d permits convexity/span's deterministic",
    ...
    if ("convexity" %in% which) {
        out$convexity_index <- if (!is.null(array_mesh)) {
            if (use_deterministic_pairwise) {
                .mesh_convexity_index(array_pieces_lazy, poly_u,
                    n_quad = n_quad, plot = FALSE,
                    weight = array_mesh$tri_weight
                )$index
            } else {
                .random_line_index(poly_u,
                    n_lines = n_lines,
                    prep = list(poly = poly_u, tri = data.frame(area = array_mesh$tri_area)),
                    seed = seed, plot = FALSE, weight = array_mesh$tri_weight,
                    points = array_pairwise_points
                )$index
            }
        } else if (use_deterministic) {
            .mesh_convexity_index(pieces, poly_u,
                n_quad = n_quad, plot = FALSE,
                weight = pieces$weight
            )$index
        } else {
            .random_line_index(poly_u,
                n_lines = n_lines, prep = list(poly = poly_u, tri = pieces),
                seed = seed, plot = FALSE, weight = pieces$weight, points = shared_points
            )$index
        }
    }
    ...
    if (any(c("moment_of_inertia", "moment_isotropy") %in% which)) {
        moi_core <- if (!is.null(array_mesh)) {
            .moment_of_inertia_core_array(
                array_mesh$P, array_mesh$T, array_mesh$tri_area,
    ...
    if ("directional_balance" %in% which) {
        out$directional_balance_index <- if (!is.null(array_mesh)) {
            if (use_deterministic) {
                .mesh_directional_balance_index_array(
                    array_mesh$P, array_mesh$T, array_mesh$tri_area,
                    array_mesh$tri_weight, array_mesh$crs
                )$index
            } else {
                .random_point_directional_balance_index_array(
                    array_mesh$P, array_mesh$T, array_mesh$tri_area,
            ...
            .random_point_directional_balance_index(poly_u,
                n_lines = n_lines, prep = list(poly = poly_u, tri = pieces),
                seed = seed, weight = pieces$weight, points = shared_points
            )$index
        }
    }
    if ("span" %in% which) {
    ...
    if ("radial_concentration" %in% which) {
        out$radial_concentration_index <- if (!is.null(array_mesh)) {
            if (use_deterministic) {
                .mesh_radial_concentration_index_array(
                    array_mesh$P, array_mesh$T, array_mesh$tri_area,
                    array_mesh$tri_weight, array_mesh$crs
                )$index
            } else {
                .random_point_radial_index_array(
                    array_mesh$P, array_mesh$T, array_mesh$tri_area,
                    array_mesh$tri_weight, array_pointcloud_points,
                    array_mesh$crs
                )$index
            ...
            .random_point_radial_index(poly_u,
                n_lines = n_lines, prep = list(poly = poly_u, tri = pieces),
                seed = seed, weight = pieces$weight, points = shared_points
            )$index
        }
    ...
    if ("depth" %in% which) {
        out$depth_index <- if (!is.null(array_mesh)) {
            if (use_deterministic) {
                .mesh_depth_index_array(
                    array_mesh$P, array_mesh$T, array_mesh$tri_area,
                    array_mesh$tri_weight, poly_u
                )$index
            } else {
                .random_point_depth_index_array(
                    array_mesh$P, array_mesh$T, array_mesh$tri_area,
                    array_mesh$tri_weight, array_pointcloud_points,
                    poly_u
                )$index
            ...
            .random_point_depth_index(poly_u,
                n_lines = n_lines, prep = list(poly = poly_u, tri = pieces),
                seed = seed, weight = pieces$weight, points = shared_points
            )$index
        }
