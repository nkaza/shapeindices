...
convex_decompose <- function(poly, prep = NULL, simplify_tolerance = NULL) {
    ...
    coords_list <- purrr::map(st_geometry(tri), \(g) st_coordinates(g)[1:3, 1:2, drop = FALSE])
    ...
    internal <- split(seq_along(all_edges), all_edges) |> purrr::keep(\(idx) length(idx) == 2)
    ...
    group <- purrr::map_int(seq_len(n_tri), find)
    grp_ids <- sort(unique(group))
    piece_geom <- do.call(c, purrr::map(grp_ids, \(g) st_union(st_geometry(tri)[group == g])))
