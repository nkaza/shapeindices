...
## intersection test (all lines x all edges at once) - ~4x faster, and
## matches the GEOS result to within ~5e-14. Falls back to the GEOS loop
## if the vectorised path errors on unexpected geometry.
##
...
## outer shape. At real-world scale (E ~ 215,000, M ~ 5,000, a genuine
## case that used to fail) M*E reaches past a billion, and the ~15
...
.boundary_edges <- function(poly) {
    ...
    p1 <- do.call(rbind, purrr::map(rings, \(r) r[-nrow(r), 1:2, drop = FALSE]))
    p2 <- do.call(rbind, purrr::map(rings, \(r) r[-1, 1:2, drop = FALSE]))
    list(p1 = p1, p2 = p2)
}
...
.frac_outside_vectorised <- function(x1, y1, x2, y2, edges, poly_u, crs, chunk_size = NULL) {
    ...
    for (cs in seq(1L, M, by = chunk_size)) {
        ...
        X1 <- rep(x1[idx], each = E)
        Y1 <- rep(y1[idx], each = E)
        RX <- rep(x2[idx] - x1[idx], each = E)
        RY <- rep(y2[idx] - y1[idx], each = E)
        EX1 <- rep(ex1, times = m)
        EY1 <- rep(ey1, times = m)
        SX <- rep(sx, times = m)
        SY <- rep(sy, times = m)
        ...
        starts <- st_sfc(purrr::map(cross_global, \(i) st_point(c(x1[i], y1[i]))), crs = crs)
...
.frac_outside_geos <- function(seg, len_total, idx_out, poly_u) {
    ...
    frac_outside[idx_out] <- purrr::map_dbl(idx_out, compute_frac)
    frac_outside
}
    ...
    result <- tryCatch(
        {
            ...
            .frac_outside_vectorised(
                x1[idx_out, 1], x1[idx_out, 2],
...
.eval_mesh_pairs <- function(qmat, q, n, poly_u, crs) {
    ...
    if (requireNamespace("sfheaders", quietly = TRUE)) {
        ...
        seg <- purrr::map(seq_len(m), \(k) st_linestring(rbind(x1[k, ], x2[k, ])))
