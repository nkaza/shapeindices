...
ellipse_of <- function(g, dx, k = sqrt(3), n = 120) {
    ...
    list(
        ell = data.frame(x = ell[, 1], y = ell[, 2]),
...
ggplot() +
    geom_polygon(
        data = grab("ring"), aes(x, y, group = shape),
        fill = "grey90", color = "grey40"
    ) +
    geom_path(
        data = grab("ell"), aes(x, y, group = shape),
        color = "firebrick", linewidth = 0.9
    ) +
    geom_segment(
        data = grab("axes"), aes(x, y, xend = xend, yend = yend),
...
pinwheel_tbl <- do.call(rbind, lapply(names(pinwheels), function(nm) {
    ...
    data.frame(
        shape = shape_thumb(pinwheels[[nm]]),
        name = nm,
