...
test_that("byrow = FALSE moment_of_inertia_index uses the true mass centroid when weighted", {
    ...
    res_wt <- shape_indices_sf(x,
        byrow = FALSE, which = "moment_of_inertia",
...
test_that("byrow = FALSE treats weight 0 and weight NA identically, as holes", {
    ...
    expect_warning(
        r_zero <- shape_indices_sf(x, byrow = FALSE, weights = c(10, 0, 10), id = "z"),
        "zero or NA weight"
    )
    expect_warning(
        r_na <- shape_indices_sf(x, byrow = FALSE, weights = c(10, NA, 10), id = "n"),
        "zero or NA weight"
    )
...
test_that("byrow = FALSE errors cleanly when every row is a hole", {
    ...
    expect_error(
        suppressWarnings(shape_indices_sf(x, byrow = FALSE, weights = c(0, 0), id = "z")),
...
test_that("byrow = FALSE weights naming the geometry column itself gives a specific error", {
    ...
    expect_error(
        shape_indices_sf(x, byrow = FALSE, weights = "geometry", id = "z"),
        "geometry column"
    )
})
...
test_that("byrow = FALSE weights rejects non-numeric input with a graceful, specific error", {
    ...
    # direct vectors
    expect_error(
        shape_indices_sf(x, byrow = FALSE, weights = c("a", "b", "c"), id = "z"),
        "must be numeric"
    )
    expect_error(
        shape_indices_sf(x, byrow = FALSE, weights = c(TRUE, FALSE, TRUE), id = "z"),
        "must be numeric"
    )
    expect_error(
        shape_indices_sf(x, byrow = FALSE, weights = factor(c("a", "b", "c")), id = "z"),
        "is a factor"
    )
...
test_that("byrow = FALSE, parallel_rows = TRUE is rejected", {
    ...
    expect_error(
        shape_indices_sf(x, byrow = FALSE, id = "z", parallel_rows = TRUE),
...
test_that("byrow = FALSE weights vector must have one entry per row", {
    ...
    expect_error(
        shape_indices_sf(x, byrow = FALSE, weights = c(1, 2), id = "z"),
        "one entry per row.*\\(3\\).*got 2"
    )
    expect_error(
        shape_indices_sf(x, byrow = FALSE, weights = c(1, 2, 3, 4), id = "z"),
...
test_that("byrow = FALSE deterministic_max_tri switches the combined mesh to the random-line estimator", {
    ...
    x <- sf::st_sf(
        name = paste0("s", seq_along(squares)),
    ...
    res <- shape_indices_sf(x,
        byrow = FALSE, id = "z", deterministic_max_tri = 1,
        n_lines = 10, seed = 1
    )
    expect_true(is.finite(res$convexity_index))
...
test_that("byrow = FALSE deterministic_max_tri can force deterministic = TRUE down to FALSE (the guardrail case)", {
    ...
    expect_warning(
        shape_indices_sf(x,
            byrow = FALSE, id = "z", deterministic = TRUE, deterministic_max_tri = 1,
            n_lines = 100, seed = 1
        ),
        "not substantially lower"
    )
...
test_that("byrow = FALSE: an explicit deterministic = FALSE overrides deterministic_max_tri, not the other way round", {
    ...
    expect_warning(
        shape_indices_sf(x,
            byrow = FALSE, id = "z", deterministic = FALSE, deterministic_max_tri = 1000,
            n_lines = 100, seed = 1
        ),
        "not substantially lower"
    )
