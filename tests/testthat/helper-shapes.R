...
# an nx*ny grid of unit squares, one row per cell, with a weight function so
# tests can control how much density varies cell-to-cell - real per-row
# density variance is what actually exercises the constrained-triangulation
# path (see shape-indices.R's dispatch: it's skipped entirely when
# weights = NULL, since density is then uniform). A helper (not a
# test-file-local function) since multiple test files use it and helper-*
# files are guaranteed to be sourced before every test file runs, regardless
# of test file load order.
