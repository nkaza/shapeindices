test_that(".frac_outside_vectorised gives identical results chunked vs unchunked", {
    ...
    set.seed(1)
    n_lines <- 40
    x1 <- runif(n_lines, -2, 12)
    y1 <- runif(n_lines, -2, 12)
    x2 <- runif(n_lines, -2, 12)
    y2 <- runif(n_lines, -2, 12)
...
test_that(".frac_outside_vectorised with default (auto) chunk_size matches an explicit large one", {
    ...
    set.seed(2)
    n_lines <- 20
    x1 <- runif(n_lines, -6, 6)
    y1 <- runif(n_lines, -6, 6)
    x2 <- runif(n_lines, -6, 6)
    y2 <- runif(n_lines, -6, 6)
...
test_that("convexity_index (deterministic = FALSE) gives the same result forcing a tiny chunk size as it does unchunked", {
    ...
    on.exit({
        assign(".choose_line_chunk_size", orig, envir = ns)
        lockBinding(".choose_line_chunk_size", ns)
    })
